...
export function generateStaticParams() {
  return (["ro", "ru"] as Locale[]).flatMap((locale) =>
    blogPosts[locale].map((post) => ({ locale, slug: post.slug })),
  );
}
function findPost(locale: Locale, slug: string) {
...
export async function generateMetadata({
  ...
  const post = findPost(locale, p.slug);
  if (!post) return {};
  const path = `/${locale}/blog/${p.slug}/`;
  const otherLocale = locale === "ro" ? "ru" : "ro";
  const otherPath = `/${otherLocale}${localizedBlogPath(`/blog/${p.slug}`, locale)}/`;
  return {
    title: `${post.title} | Volmer`,
  ...
  const jsonLd = [
    {
      ...
      publisher: { "@type": "Organization", name: "Volmer" },
    ...
    },
    {
      ...
      itemListElement: [
        ...
        },
        {
          ...
          position: 2,
          name: "Blog",
          item: `${siteConfig.siteUrl}/${locale}/blog/`,
        },
  ...
  return (
    <article className="container page blog-page">
      ...
      <p className="article-disclaimer">
        {locale === "ro"
          ? "Informațiile sunt generale și nu înlocuiesc o evaluare individuală a auzului."
          : "Информация носит общий характер и не заменяет индивидуальную оценку слуха."}
      </p>
      <div className="actions">
        ...
        >
          {locale === "ro" ? "Sună pentru programare" : "Позвонить для записи"}
        </a>
        <a className="button ghost" href={`/${locale}/aparate-auditive/`}>
          {locale === "ro" ? "Aparate auditive" : "Слуховые аппараты"}
        </a>
      </div>
      <section className="related">
        <h2>
          {locale === "ro" ? "Articole recomandate" : "Рекомендуемые статьи"}
        </h2>
        {related.map((item) => (
          <p key={item!.slug}>
